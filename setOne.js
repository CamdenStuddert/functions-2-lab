...
const testFunc = (param1, param2) => console.log(`The first parameter is ${param1}. The second parameter is ${param2}`)
...
greeting(`Camden`, `Studdert`, fullName)
...
const pricesPlusTax = (arr, callback) => {
    for(let i = 0; i < arr.length; i++){
        let totalTax = arr[i] + (arr[i] * .2)
        totalCost.push(totalTax)
    }
    callback(totalCost)
}
...
function confusing(totalCost){
    console.log(`The new array plus tax = ${totalCost}`)
}
pricesPlusTax(prices, confusing)
